...
public class MyBlockQueue<E> {
	...
	public E take() throws Exception {
		try {
			...
			while (queue.size() <= 0) {
				System.out.println(Thread.currentThread().getName() + " 因为不满足不空的条件阻塞了");
				notEmpty.await();
				System.out.println(Thread.currentThread().getName() + " 队列有数据了被唤醒了,while内");
			}
			System.out.println(Thread.currentThread().getName() + " 队列有数据了被唤醒了,while外");
			E e = queue.remove();
